---
name: prd-creator
description: Convert markdown PRDs to executable JSON format. Use after creating a PRD with the prd-taskmaster to generate the prd.json for autonomous task completion.
---
...
4. Create `prd/<prd-name>/` directory
5. Move markdown PRD to `prd/<prd-name>/prd.md`
6. Output JSON to `prd/<prd-name>/prd.json`
7. Create empty `prd/<prd-name>/progress.txt`
...
Move PRD and generate JSON in `prd/<prd-name>/`:
...
```
PRD converted and moved to prd/<prd-name>/
  - prd.md
  - prd.json
  - progress.txt
